...
const App = () => {
  ...
  const errorString =
    'Invalid Search Criteria.  Valid searches are:\n' +
  ...
  const Find_Provider_Data = () => {
    ...
    if (CheckValid) {
      ...
      if (iNum > 0 && ICDisplayErrorBoxAndTest) {
        //if the length of some fields are less this error pops
        console.log('field error');
        //setting usestate hooks that are send to Modal component as props
        SetErrorMessage(
          'GrpHlth Warning ' +
            ICDisplayErrorBoxAndTest +
            ' Name for ' +
            csType +
            ' Search must have ' +
            iNum +
  ...
  const LnameSelect = e => {
    ...
    if (e.target.value.length >= 1) {
      //if lname field has any value disable all other fields except fname
    ...
    <div className="wrapper">
      ...
      <div className="Top_Component">
        <div className="All_Search">
          <div className="Search_by_name">
            <form className="Search_by_name_form">
              <fieldset className="Search_by_name_fieldset">
                <legend>Search by Name</legend>
                <label>Facility/Group:</label>
                <input
                  type="text"
                  className="Seach_facility"
                  id="Seach_facility"
                  value={GroupName}
                ...
                <input type="checkbox" disabled={GroupToggle}></input>
                <br></br>
                <label>Last Name</label>
                <input
                  type="text"
                  className="Seach_Lname"
                  id="Seach_Lname"
                  value={Lname}
                  disabled={LnameToggle}
                  onChange={LnameSelect}
                ></input>
                <br></br>
                <label>Soundex search</label>
                <input type="checkbox" disabled={LnameToggle}></input>
                <br></br>
                <label>First Name</label>
                <input
                  type="text"
                  className="Seach_Fname"
                  id="Seach_Fname"
                  value={Fname}
                  disabled={FnameToggle}
                ...
                <input
                  type="text"
                  className="Seach_TaxID"
                  id="Seach_TaxID"
                  value={TaxID}
                  disabled={TaxIDToggle}
                  onChange={TaxIDSelect}
                ></input>
                <br></br>
              </fieldset>
            </form>
          ...
          <div className="Search_for">
            <fieldset className="Search_for_fieldset">
              <legend>Search for</legend>
              <input
                type="radio"
                className="search_for_all"
                id="All"
                name="search_for_"
                value="All"
                onChange={e => SetSearchFor(e.target.value)}
                disabled={SearchForToggle}
              ></input>
              <label>All</label>
              <input
                type="radio"
                className="search_for_Practioners"
                id="Practioners"
                name="search_for_"
                value="Practioners"
                onChange={e => SetSearchFor(e.target.value)}
                disabled={SearchForToggle}
              ></input>
              <label>Practioners</label>
              <br></br>
              <input
                type="radio"
                className="search_for_non-Practioners"
                id="Non-Practioners"
                name="search_for_"
                value="Non-Practioners"
                onChange={e => SetSearchFor(e.target.value)}
                disabled={SearchForToggle}
              ></input>
        ...
        <div className="Service_line">
          <fieldset className="Service_line_fieldset">
            <legend>Service Line</legend>
            <label>Primary Care Specialty</label>
            <select
              className="Service_line_Primary_Care"
              value={PrimaryCare}
            ...
            <select
              className="Service_line_Other_medical"
              value={OtherMedical}
            ...
            <select
              className="Service_line_Mental_Health"
              value={MentalHealth}
            ...
            <br></br>
            <label>Service Lines Selected</label>
            <br></br>
            <div className="del_btn">
              <textarea
                rows="5"
                clos="10"
                className="Service_line_selected"
              ></textarea>
        ...
        <div className="Location">
          <fieldset className="Location_fieldset">
            <legend>Location</legend>
            <label>Search Radius (in miles)</label>
            <input
              type="text"
              className="Location_Radius"
              id="Location_Radius"
              value="0"
              disabled={false}
            ></input>
            ...
            <input
              ...
              id="Location_City"
              value={LocationCity}
              onChange={e => SetLocationCity(e.target.value.toUpperCase())}
              disabled={false}
            ></input>
            <br></br>
            <label>County</label>
            ...
            <br></br>
            <button className="Location_use_Member">
              Use Member's Address
            </button>
            <button className="LocationClear" onClick={ClearAddress}>
              Clear
            </button>
          </fieldset>
        </div>
      ...
      <div className="Find_clr_btn">
        ...
        <button className="Btn_Find" onClick={Find_Provider_Data}>
          Find
        </button>
        <button className="Btn_Clear" onClick={GlobalAndTable_clear}>
          Clear
        </button>
      </div>
